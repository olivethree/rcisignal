...
own image, composited over a base face. The same call works for
...
The default palette is `"diverging"` (matches `plot_ci_overlay()`, blue
= positive, red = negative). Pass `palette = "fire"` for the unipolar
`|t|`-style yellow-to-red ramp, `format = "jpeg"` to write JPEGs
instead, or `prefix = "trust_"` (or any other string) to override the
auto-derived prefix.
