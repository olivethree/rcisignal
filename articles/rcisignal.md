...
reproduces Oliveira et al. (2019) Table 2 to within ±0.02 across 17 of
18 spot-checked cells; the one exception is trustworthy–untrustworthy,
where the package recovers −0.57 against the paper’s −0.68.
Same-dimension opposite-pole pairs are the most sensitive to small
differences in the underlying CIs, so the most plausible source is a
small producer-set or pipeline difference between the 2019 analysis
(rcicr v0.3.0 directly) and the cached signal matrices used here
(rebuilt from the OSF responses).
