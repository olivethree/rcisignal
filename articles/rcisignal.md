...
| `images_per_trial` (Brief-RC only) | `12` | = 6 oriented/inverted pairs |
...
- **Brief-RC.** Each trial shows `images_per_trial = 2k` images (the
  oriented and inverted versions of `k` distinct noise patterns drawn
  ...
  is `+1` if the oriented version of that pair was chosen, `-1` if the
  inverted version.
