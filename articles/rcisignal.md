...
to confuse, and `rcisignal`’s API uses argument names that look alike
(`noise_matrix`, `signal_matrix`, `mask`). Each one has exactly one job:

| name | what it is | shape | input or output? |
|----|----|----|----|
| **`noise_matrix`** | the **input** pool of pre-generated noise patterns the experiment chose stimuli from. One column per pool pattern. | `n_pixels` × `pool_size` | input (you supply it; §4.3) |
| **noise mask** (a.k.a. *raw mask*, *per-participant CI*) | **one participant’s** classification image: a single vector of pixel values, base-subtracted. Conceptually the weighted average of the noise patterns that participant “selected” with their responses. The word *mask* here means *image-shaped overlay* — it is **not** a logical face-region mask (those are §4.5). | `n_pixels` × 1 | intermediate |
| **`signal_matrix`** | **all participants’ noise masks stacked side by side**, one column per producer. The central object of `rcisignal`; every reliability/discriminability function consumes a matrix of this shape. | `n_pixels` × `n_participants` | output (you pass it downstream) |

Plain-English summary:

- The **`noise_matrix`** describes the *stimuli* (what the experiment
  showed). It exists *before* any participant runs the task. §4.3 covers
  how to read it.
- A **noise mask** describes *one participant’s* CI. It exists *after*
  that participant has responded. You normally do not handle it
  directly; it is one column of the `signal_matrix`.
- The **`signal_matrix`** describes *the whole sample’s* CIs. It is the
  collection of all individual noise masks lined up by participant —
  what every `rel_*`,
  [`run_reliability()`](https://olivethree.github.io/rcisignal/reference/run_reliability.md),
  [`run_discriminability()`](https://olivethree.github.io/rcisignal/reference/run_discriminability.md),
  [`infoval()`](https://olivethree.github.io/rcisignal/reference/infoval.md),
...
Each trial presents 12 noisy faces (6 original, `base + noise_i`, and 6
...
original chosen, `-1` if inverted. Unselected faces are absent from the
...
[`ci_from_responses_briefrc()`](https://olivethree.github.io/rcisignal/reference/ci_from_responses_briefrc.md)
accepts `method = "briefrc12"` (12 alternatives per trial; 6 original, 6
